...
/** An instance represents a virus spreading and ultimately dying out among<br>
 * a limited population (or killing everyone). <br>
 * <br>
 * Each Virus is created on a Network of people and with a chosen first patient. <br>
 * Virus is runnable, but for the purposes of this project, it does not need to <br>
 * be run on a separate thread.
 *
 * @author MPatashnik, revised by gries. */
public class Virus implements Runnable {
	/** The graph on which this Virus is running. */
	private Network network;

	/** The tree representing this virus spreading. */
	private VirusTree tree;

	/** Number of time steps this virus took to create dt. */
	private int steps;

	/** The virus model: Statistics that determine the spread of the disease. */
	...
	/** Constructor: a new Virus on network nw with first patient fp and disease model s. */
	public Virus(Network nw, Person fp, Statistics s) {
		steps= 0;
		network= nw;
		fp.getSick(0);
		tree= new VirusTree(fp);
		statistics= s;
	}
	...
	private void step() {
		...
		for (Person p : people) {
			if (p.isSick()) {
				...
				if (n != null && n.isHealthy() && statistics.virusSpreadsToPerson()) {
					n.getSick(steps);
					tree.insert(n, p);
				}
			}
	...
	/** Run Virus on the arguments listed in args. <br>
	 * If args doesn't match the pattern below, read in arguments via the console by using
	 * readArgs().
	 *
	 * Then, call disease.run() and create a VirusFrame showing the created VirusTree.
	 *
	...
	public static void main(String[] args) {
		...
		// Set defaults and create the Network, Statistics, and Virus objects
		System.out.print("\nSetting up ");
		System.out.print(".");
		...
		System.out.print(".");
		Virus d= new Virus(n, Util.randomElement(n.vertexSet()), s);
		System.out.println("Done.");
		...
		VirusFrame.show(d.tree, d.steps);
	}
}
