...
/** An instance is a JFrame with a virus-spreading tree and a panel to display information
 *
 * @author Mshnik */
public class VirusFrame extends JFrame {
	...
	/** Return a new VirusFrame to display dt and s, the number of time steps to create it. */
	public static VirusFrame show(VirusTree dt, int s) {
		return new VirusFrame(dt, s);
	}
	...
	/** VirusTree being displayed. */
	private VirusTree root;
	...
	/** GraphPanel that shows the VirusTree. */
	...
	/** Display the virus route from the previously selected person to the selected person. */
	private JLabel twoPersonVirusRouteLabel= new JLabel();
	...
	/** The virus route from the previously selected person to the currently selected person. */
	private List<Person> virusRoutePrevToSelected;

	/** Constructor: a visible GUI for dt, with s time steps needed to construct it. */
	private VirusFrame(VirusTree dt, int s) {
		super();
		setTitle("A4 VirusTree Explorer");
	...
	private void createAndAddCircleMouseListener() {
		graphPanel.addCircleMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				try {
					virusRoutePrevToSelected= root.getNode(previouslySelectedPerson)
						.virusRouteTo(selectedPerson);
	...
	private void fixGraphPanel(VirusTree dt) {
...
//		infoPanel.setLayout(new BoxLayout(infoPanel, BoxLayout.Y_AXIS));
		...
		infoPanel.add(twoPersonVirusRouteLabel);
		infoPanel.add(twoPersonCommonAncestorLabel);
	}

	/** Lay out the nodes in the VirusTree. Specifically, call recLayoutNodes(...) [below], with
	...
	private void recLayoutNodes(VirusTree dt, float xMin, float xMax, float yPercent,
		float yInc) {
		...
		for (VirusTree child : dt.copyOfChildren()) {
	...
	/** Return a stroke (for drawing) for line l. If the line is part of the virusRoute, <br>
	 * return a bolded stroke; otherwise, a simple stroke. */
	private Stroke getStrokeFor(GraphPanel<Person, PersonConnection>.Line l) {
		if (virusRoutePrevToSelected != null) {
			Person p1= l.getC1().getRepresents();
			Person p2= l.getC2().getRepresents();
			if (virusRoutePrevToSelected.contains(p1) &&
				virusRoutePrevToSelected.contains(p2)) {
	 ...
	 * IMMUNE_COLOR */
	private void colorNodes(VirusTree dt) {
		GraphPanel<Person, PersonConnection>.Circle c= graphPanel.getCircle(dt.getRoot());
	...
	private void populateLabels() {
		...
		populateLabel(() -> root.getNode(p).childrenSize(), selectedPersonChildrenCountLabel,
			"Children");
		populateLabel(() -> root.getNode(p).size(), selectedPersonSubtreeCountLabel,
			"Subtree Size");
		populateLabel(() -> root.getNode(p).maxDepth(), selectedPersonMaxDepthLabel,
			"Subtree depth");
		populateLabel(() -> root.getNode(p).maxWidth(), selectedPersonMaxWidthLabel,
			"Subtree width");
		...
		if (pp != null) {
			populateLabel(() -> {
				List<Person> route= root.getNode(pp).virusRouteTo(p);
				if (route == null) return null;
				else return Arrays.deepToString(route.stream().map(a -> a.name()).toArray());
			}, twoPersonVirusRouteLabel, "Virus Route");
			populateLabel(() -> {
